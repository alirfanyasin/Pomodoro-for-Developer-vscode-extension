...
function startPomodoroSession() {
    vscode.window.showInformationMessage("Pomodoro started! 25 minutes of focus.");
    let timeLeft = 25 * 60;
    // let timeLeft = 1 * 10;
    ...
    currentTimer = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            ...
            vscode.window.showInformationMessage("Time's up! Take a 5-minute break 🍵");
            startBreakSession();
        }
    }, 1000);
}
function updateStatusBar(timeLeft) {
    const minutes = Math.floor(timeLeft / 60);
    const seconds = timeLeft % 60;
    statusBarItem.text = `🍅 ${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`;
}
function startBreakSession() {
    let panel = vscode.window.createWebviewPanel("breakTimer", "Break Time ⏳", vscode.ViewColumn.One, {
        enableScripts: true,
        localResourceRoots: [
    ...
    panel.webview.onDidReceiveMessage((message) => {
        if (message.command === "done") {
            ...
            panel.dispose();
            if (isRunning) {
                vscode.window.showInformationMessage("Break is over! Starting next Pomodoro 🧠");
                startPomodoroSession();
            }
        }
    });
...
function getBreakHtml(duration, panel) {
    const imageList = ["nature1.jpg", "nature2.jpg", "nature3.jpg"];
    const randomImage = imageList[Math.floor(Math.random() * imageList.length)];
    const imageUri = panel.webview.asWebviewUri(vscode.Uri.joinPath(contextGlobal.extensionUri, "media", randomImage));
    const audioUri = panel.webview.asWebviewUri(vscode.Uri.joinPath(contextGlobal.extensionUri, "media", "sound.mp3"));
    return `
<!DOCTYPE html>
<html lang="en">
<head>
